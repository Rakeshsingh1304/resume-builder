...
function Heading({ children }: { children: React.ReactNode }) {
    return (
        <h2 className="text-[13px] font-bold uppercase tracking-wider text-[#2563EB] border-b-2 border-[#2563EB]/20 pb-1.5 mb-3">
            {children}
        </h2>
    );
}
    ...
    return (
        ...
        >
            ...
            <div className="w-[270px] shrink-0 bg-[#14213D] text-white p-7" style={{ breakInside: "avoid" }}>
                ...
                {personalInfo.title && (
                    <p className="text-[13px] text-[#93C5FD] mt-1">{personalInfo.title}</p>
                )}

                <div className="mt-7">
                    <h2 className="text-[11px] font-bold uppercase tracking-wider text-[#93C5FD] mb-2.5">
                        Contact
                    </h2>
                    <div className="space-y-2 text-[11.5px] text-gray-200">
                        {personalInfo.phone && (
                            <div className="flex items-center gap-2">
                                <Phone size={12} className="text-[#93C5FD] shrink-0" />
                                <span className="break-all">{personalInfo.phone}</span>
                            </div>
                        )}
                        {personalInfo.email && (
                            <div className="flex items-center gap-2">
                                <Mail size={12} className="text-[#93C5FD] shrink-0" />
                ...
                {skills.length > 0 && (
                    <div className="mt-7">
                        ...
                        <ul className="space-y-1.5 text-[11.5px] text-gray-200">
                            {skills.map((skill) => (
                        ...
                        {experience.map((exp) => (
                            <div key={exp.id} className="mb-4 last:mb-0" style={{ breakInside: "avoid" }}>
                                <div className="flex justify-between items-baseline">
                                    <p className="font-bold text-[13.5px] text-[#14213D]">
                                        {exp.role} {exp.company && <span className="font-normal text-gray-600">| {exp.company}</span>}
                                    </p>
                                    ...
                                    <ul className="mt-1.5 space-y-1">
                                        {exp.description.split("\n").filter(Boolean).map((line, i) => (
                                            <li key={i} className="text-[12px] text-gray-700 flex gap-1.5 leading-relaxed">
                                                <span className="text-[#2563EB]">•</span> {line}
                                            </li>
                                        ))}
                                    </ul>
                                )}
                    ...
                    <div className="mb-5">
                        ...
                        {projects.map((proj) => (
                            <div key={proj.id} className="mb-3.5 last:mb-0" style={{ breakInside: "avoid" }}>
                                <div className="flex justify-between items-baseline">
                                    <p className="font-bold text-[13px] text-[#14213D]">{proj.title}</p>
                                    {proj.link && (
                                        <p className="text-[11px] text-[#2563EB] whitespace-nowrap ml-3">{proj.link}</p>
                                    )}
                                </div>
                                {proj.techStack && (
                    ...
                    <div className="mb-5" style={{ breakInside: "avoid" }}>
                        ...
                        {education.map((edu) => (
                            <div key={edu.id} className="mb-2.5 last:mb-0">
                                <div className="flex justify-between items-baseline">
                                    <p className="font-bold text-[13px] text-[#14213D]">
                                        {edu.degree}
